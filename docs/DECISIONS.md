...
the credential is the `Auth` config section:

- `Auth__PasswordHash` (production) — PBKDF2-SHA256, format
  `v1.{iterations}.{salt}.{hash}`, generated by `dotnet run -- hash-password`.
  `Auth:Password` (plaintext) is a dev-only fallback, lives in the gitignored
  `appsettings.Development.json`.
- No self-registration, no reset flow, no user table. Changing the password =
  regenerate the hash + restart. Named accounts are a backlog item if usage
  grows.

**Everything is behind the login.** `[Authorize]` in `_Imports.razor` + an
`AuthorizeRouteView` in `Routes.razor` whose `NotAuthorized` renders
`RedirectToLogin` (full-page nav to `/login?returnUrl=…`). `Login` / `Error` /
`NotFound` opt out with `[AllowAnonymous]`. An authorization **fallback policy**
(`RequireAuthenticatedUser`) covers the non-component endpoints — critically the
five document endpoints, which otherwise would have streamed PDFs/XLSX (full
product data) to anyone. Static assets and the Razor-components endpoint are
explicitly `.AllowAnonymous()` (the component gate is `AuthorizeRouteView`, not
the endpoint).

**Login is static SSR, not interactive.** Setting an auth cookie needs a real
HTTP response, which a SignalR circuit can't do. `Login.razor` is a plain
`<form method="post" action="/auth/login">` (+ `<AntiforgeryToken/>`) rendered
with no `@rendermode`; the endpoint calls `HttpContext.SignInAsync` and
`LocalRedirect`s to the (validated, local-only) return URL. Logout is a `POST`
